...
int main(void) { //프로그램은 항상 메인 함수로부터 시작한다,
	...
	*/
	...
	증감연산자
	++i, --i  i의 값을 1증가(혹은 감소)시킨 후에 증가(혹은 감소)된 값을 반환
	i++, i--  i의 값을 1증가(혹은 감소)시킨 후에 증가(혹은 감소)되기 전의 값을 반환
	i의 위치는 출력순서 라고 생각하면 딱임.

	삼항연산자
	(a == b) ? 100 : -100    a와 b가 같으면 100 아니면 -100 출력
